...
function initServicesSelector() {
    ...
    if (servicesSelectorBtn && servicesMenu && servicesSelector) {
        ...
        servicesSelectorBtn.addEventListener("click", function (e) {
            ...
            if (isHidden) {
                // Pozicionálás számítása csak akkor, ha nincs CSS-ben beállítva
                // A service oldalon a CSS-ben van beállítva a pozíció, ne írjuk felül
                if (!servicesMenu.style.right && !servicesMenu.style.top && !servicesMenu.style.left) {
                    const rect = servicesSelectorBtn.getBoundingClientRect();
                    servicesMenu.style.left = rect.left + 'px';
                    servicesMenu.style.top = (rect.bottom + 8) + 'px';
                }
